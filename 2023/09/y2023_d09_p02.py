...
def solve():
    ...
    for nums in numbers:
        print(nums)
        latest = [y-x for (y,x) in zip(nums[1:], nums[:-1])]
        print(latest)
        wew = [nums, latest]
        while not all(x == 0 for x in latest):
            good = [(y,x)  for y,x in zip(latest[1:], latest[:-1])]
            latest = [y-x for (y,x) in good]
            wew.append(latest)
