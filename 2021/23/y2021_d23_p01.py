...
DESIRED = {"A": {(2,3), (3,3)}, "B": {(2,5),(3,5)}, "C": {(2,7), (3,7)}, "D": {(2,9),(3,9)}}
...
def get_pods_moves(pods, grid):
    ...
    for pod, src in pods.items():
        ...
        if pods[pod[0]+"1"] in DESIRED[pod[0]] and pods[pod[0]+"2"] in DESIRED[pod[0]]:
            # print("We done")
            gmo[pod] = costs
            continue
        ...
        for x in [3,5,7,9]:
            ...
            if DESIRED_COL[pod[0]] != x:
                ...
                p1 = (2,x)
                p2 = (3,x)
                if p1 in costs and (not p2 in costs):
                    gv = pod[0] + {"1": "2", "2": "1"}[pod[1]]
                    if pods[gv] != p2:
                        del costs[p1]
    ...
    while len(Q) > 0:
        ...
        for pod, costs in gmo.items():
            for place, mcost in costs.items():
                npods = pods.copy()
                npods[pod] = place
                heapq.heappush(Q, (cost + mcost,to_tup(npods)))
