...
class State():
    ...
    def score(self):
        f1, f2, f3, f4 = [len(x.gens) + len(x.chips) for x in self.floors]
        return 1000000 * f1 + 100000 * f2 + 1000 * f3 + 10*(len(self.e.gens) + len(self.e.chips))
        ...
        for l in range(1,3):
            for combs in it.combinations(all_together, l):
                ...
                for n in upad:
                    ...
                    ee = Elevator(self.e.floor + n, [x for x in combs if isinstance(x, Generator)], [x for x in combs if isinstance(x, Chip)])
    

                    f1, f2, f3, f4 = self.floors
                    fn = Floor(tuple(sorted([x for x in thangs if isinstance(x, Generator)])), tuple(sorted([x for x in thangs if isinstance(x, Chip)])))
